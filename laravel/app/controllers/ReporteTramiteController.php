        ...
        foreach ($rst as $ind => $ndc){
                ...
                foreach ($new as $dFile) {
                        ...
                        if($found!==false || ($c1 !== false && $c2 !== false)){
                            ...
                            $rst[$ind]->referido .= ' <b><a href="javascript:window.open(atob(\''.base64_encode(str_replace(array('°','°'), array('%B0','%B0'), $dFile)).'\'));"<i class="fa fa-video-camera"><input type="hidden" id="vid_'.($ind+1).'" value="'.base64_encode($dFile).'"> </i></a></b>';
                            //var_dump($rst[$ind]);
                        }
                }
        }
