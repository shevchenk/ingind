...
/*
          ...
          $this->sarrayToCsv("php://output",(array)$rst);
   ...
   {
        $oData=AuditoriaAcceso::CuadroAuditoriaAcceso();
        return Response::json(
            array(
                'rst'=>1,
                'datos'=>$oData['data'],
           ...
           public function postDetallecuadroproceso()
   {
    ...
    public function postReportetramiteactividad()
    {
      ...
      $oData = Reporte::ReporteTramiteActividad( $array, $data ); //$data=$cant_pasos
...
/*          if($result){
   ...
   /*   if( Request::ajax() ){*/
    ...
    public function postObtenerqr() {
      $size = 140; // TAMAÑO EN PX
      $png = QrCode::format('png')->margin(0)->size($size)->generate("http://proceso.munindependencia.pe/documentodig/vistauserqr/".Input::get('rol_id')."/".Input::get('area_id')."/".Input::get('dni')."/".Input::get('tamano')."/".Input::get('tipo'));
